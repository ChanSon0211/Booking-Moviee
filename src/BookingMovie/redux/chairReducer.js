    ...
    chairList: [

        {
            ...
            "danhSachGhe": [
                ...
                { "soGhe": "A12", "gia": 750000, "daDat": false }
            ]
        },
        {
            "hang": "B",
...
export const chairReducer = (state = initialState, action) => {
    switch (action.type) {
        case "CHANGE_CHAIR": {
            console.log(action);
            const data = [...state.chairSelecetedList]

            // kiểm tra trong mảng chairSelecetedList có tồn tại ghế hay chưa 
            ...
            return { ...state, chairSelecetedList: data }

        }
        case "PAY": {
            const data = state.chairList.map(item => {
              const dsGhe =  item.danhSachGhe.map(ghe => {
                    state.chairSelecetedList.forEach(ele => {
            ...
            return {...state , chairList : data , chairSelecetedList:[]}
        }
        default
            : return state
    }
